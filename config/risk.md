...
- Any tier increase must be made manually in `config/runtime.env`; Codex must not raise `RISK_TIER` by itself.

Hard limits:
- Only trade symbols that are present in all applicable gates:
  - `config/universe.txt`;
  - `state/today_allowlist.txt`;
  - `state/daily_plan.json.today_watchlist`.
- `config/allowlist.txt` is only a static emergency fallback and must not be used for normal dynamic trading.
...
- Max single order notional is the lower of `config/runtime.env`, `RISK_TIER`, and `state/daily_plan.json`.
- Max daily notional is the lower of `config/runtime.env`, `RISK_TIER`, and `state/daily_plan.json`.
...
- If `state/today_allowlist.txt` is missing, stale, empty, or not generated today, do not place any order.
