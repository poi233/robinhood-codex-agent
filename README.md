...
| `analytics screen-eval [--since --until]` | O4 → `screen_eval_report.{json,md}`: selection-layer effectiveness — O1 added-symbol forward returns + excess vs SPY, demoted-symbol returns, and `screen_score` Rank IC. Empty (`insufficient_data`) until the weekly screener has applied and future bars exist. |
