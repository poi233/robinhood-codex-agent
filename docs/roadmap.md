...
| | H2 | **价量因子层**（factors_price + factor_alpha + factor_profiles，ChatGPT Phase 2） | ChatGPT | ✅ **代码已完成（2026-06-16，flag 默认 0）**；**待你在 paper 设 `ENABLE_PRICE_FACTOR_LAYER=1` 攒数据** |
...
- premarket：`run_price_factors` 并行 local stage，**`ENABLE_PRICE_FACTOR_LAYER` 默认 0**（关时 no-op、
  premarket 一字不变）；**只写两个新产物、不进 champion 打分**。`doctor` 回显 flag；新增 RuntimePaths
  `factor_panel_path`/`factor_alpha_path`。
...
- 测试：factors_price 6、factor_alpha 7、factor_layer_integration 2、calibration pickup/动态分桶若干、
  dashboard query/render；**flag 关时既有测试逐字不变全绿（372）**。

**完成门槛已达**（计算+落盘+factor_alpha+校准 pickup+dashboard+测试+doctor 全做完）。按约定 **repo 默认仍
0**；**请在 paper 的 `src/config/runtime.env.local` 设 `ENABLE_PRICE_FACTOR_LAYER=1`** 开始攒 point-in-time
因子数据（只写、不影响交易）。稳定后再做 flag 清理任务。
