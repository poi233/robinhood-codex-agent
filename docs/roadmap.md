...
| M1–M5 intraday advisory overlay | `ENABLE_INTRADAY_ADVISORY_OVERLAY` | intraday 排序/仓位/风控 overlay | ⏳ **规划中（2026-06-18，默认 0）**：把 H2/H3/K1/K2 产物受控接进 intraday；flag 关时 champion 完全保持旧行为 |
...
| **M Advisory Overlay 接线（用户 D 方案）** | M1 | intraday 读取 advisory artifacts + overlay 归一化 | 用户新增 | ⏳ 规划中：新增 `policy/advisory_overlay.py`，读取 `factor_alpha`/`ai_signals`/`portfolio_target`/`regime_state`，输出 rank/block/size/reason overlay |
...
| | M4 | 决策日志和 dashboard 显示每个因子如何影响最终决策 | 用户新增 | ⏳ 规划中：落 `advisory_overlay` reason codes / multipliers / rank deltas；dashboard 增 Decision Overlay 视图，便于 replay 和人工审计 |
...
3. **P2 · 冻结跑 paper 15–30 天** — 不调 champion 权重/阈值（否则样本不可比），自成长只 `observe`+`propose`；
   M 阶段可以写代码和审计日志，但 `ENABLE_INTRADAY_ADVISORY_OVERLAY=0` 时必须完全不改变 intraday 行为。
...
## M1 — Intraday advisory overlay loader + 归一化
