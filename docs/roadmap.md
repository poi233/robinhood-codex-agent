...
| **K 组合与归因（评审一·真缺口）** | K1 | **Portfolio Layer**（cash/theme/sector exposure + 单仓上限目标） | 评审一 | 🔴 **最大缺失**：当前 5 个 AI infra 票=一笔交易；建议数据积累期并行做（非 alpha 热路径） |
| | K2 | **量化 Market Regime 引擎**（Bull/Neutral/RiskOff/Panic + 仓位乘子） | 评审一 | 🔴 缺口：现 regime 是 LLM 在 daily_plan 定；升级为确定性引擎（SPY/QQQ/VIX/breadth → multiplier） |
...
| | L3 | H2 factor **benchmark coverage 审计** | 评审二 | ⏳ P1：保证 SPY/QQQ/SMH/IWM 永远在 market_feed，别依赖 watchlist 恰好含 SPY（否则 beta/residual/relative 因子 partial） |
...
| | L5 | premarket factor-failure advisory 测试 | 评审二 | ⏳ 小：断言因子层抛异常时 premarket 仍完成、daily_plan/risk_overlay 仍生成、该步记 advisory_failed |
...
**验收**：每个 run 产 `portfolio_target.json`；theme/sector 敞口与单仓上限可在 dashboard 看；第一版不改交易行为。

## K2 — 量化 Market Regime 引擎 — 🔴 高优先级

**问题**：当前 `market_regime`（risk_on/mixed/risk_off/no_trade）是 **LLM 在 `final_planner` 里定**的（opinion），
risk_overlay 也有一个基于候选分数的粗 regime。缺一个**确定性、可回测、可校准**的量化引擎。

**具体**：新增 `regime/` 模块，纯函数输入市场宽度指标（SPY/QQQ 趋势、VIX、breadth、dollar、treasury——先用
yfinance 能取到的子集，缺的标 None 降级），输出**离散 regime + 仓位乘子**：
```
Bull 1.2x · Neutral 1.0x · RiskOff 0.5x · Panic 0.0x
```
**接入分两步**：① 第一版只落盘 `regime_state.json` + 进 dashboard/calibration（与 forward returns 对照，验证
regime 是否真的改善收益曲线）；② 校准后再让 sizing 用 multiplier（**只缩不放**，Panic=0 等于 no-trade gate）。
**红线**：multiplier 只能 ≤ 现状（降风险方向），不引入杠杆。
...
## L2 — 完整 smoke checklist → `docs/smoke-test.md` — ⏳ P1

把一条龙集成验证写成可跑清单 + 文档：`doctor` → `check_safety.sh` → 干跑 premarket/intraday/postmarket →
`analytics build/calibrate/ai-signal-study/ai-ablation` → `growth observe/propose/validate/shadow/evaluate`。
也并入 [`daily-strategy-playbook.md`](./daily-strategy-playbook.md)。**验收**：清单可一键跑通，失败步骤清晰可见。

## L3 — H2 factor benchmark coverage 审计 — ⏳ P1

`factor_store.py` 从 `market_feed/ohlcv/<symbol>/daily.json` 读 daily bars、benchmark 默认 SPY。**风险**：
market_feed 只跑 active_watchlist 时，若某 strategy watchlist 不含 SPY，则 beta/residual/relative 因子 partial。
**具体**：明确保证 market_feed 采集**永远包含 benchmark 集（SPY/QQQ/SMH/IWM）**，不依赖 watchlist 恰好含 SPY；
dashboard/calibration 显示 factor coverage %（多少 active_symbol 有 daily bars）。**验收**：benchmark bars 恒在；
coverage 可见。
...
## L5 — premarket factor-failure advisory 测试 — ⏳ 小

加测试锁定：因子层（或任一 advisory 信号层）抛异常时 → premarket 仍完成 → 该 stage 记 `advisory_failed` →
`daily_plan` / `risk_overlay` 仍正常生成。确认 `_run_advisory` 包裹真的生效。**验收**：该测试通过。
