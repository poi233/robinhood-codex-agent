...
| **N 数据存储强化（空数据期红利）** | N1 | analytics.db schema 漂移修复（补全新落盘字段 + 新表） | 用户新增（2026-06-18） | 🔜 **规划中**：让 db schema/loaders 跟上 E4/E3/K3/M 的新字段，并补 factor/regime/portfolio/thesis 表 |
| | N2 | analytics.db 索引 | 用户新增（2026-06-18） | 🔜 **规划中**：run_date 等常用过滤列建索引，防数据增长后全表扫描 |
...
## N1 — analytics.db schema 漂移修复 — 🔜 规划中
...
**目标**：`runtime/state/runs/*` 和 `runtime/logs/runs/*` 会随每个交易日无限增长。现在（3 个空 run）远没到
需要治理的规模，**故意后置**，等真跑了几个月、目录变大再做。

**候选做法（届时再定）**：旧 run 的大产物（charts PNG、全量 OHLCV）压缩或移到 `archive/`；db 是派生视图可
随时从保留的 JSON 重建，所以归档时优先保 `decisions.jsonl` / `orders.jsonl` / `candidate_scores.json` /
`run_manifest.json`（calibration 的最小输入集），charts 等可丢。需保证 `discover_run_dates` /
`analytics build` / calibration 在「部分 run 已归档」时仍正确降级。

**涉及文件**：待定（新增 `analytics/retention.py` 或入口脚本）。

**验收**：待定。**现在不做**——列在此仅为不遗忘。

## N 段推荐执行顺序

1. **N1 schema 漂移**——价值最高、最该趁空数据做（有数据后改要重 build + 验证）。
2. **N2 索引**——一次性便宜，和 N1 一起做最省事（都改 build/schema）。
3. **N3 校验**——真数据进来前就位，第一天就能发现脏数据。
4. **N4 保留策略**——数月后，目录真变大再做。
