...
| | H3 | AI signal 结构化 + 归因 + ablation（ChatGPT Phase 3） | ChatGPT | 🟡 **step 1+2 已完成（2026-06-17）**：标准化 AI 信封 + 校验 + normalizer + `ai_signals.json`（write-only advisory，无 flag）；`analytics ai-signal-study`（confidence calibration / 方向准确率 / code lift）；剩 step 3 ai-ablation |
...
| **Phase 3 AI signal study** | 🟡 **step 1 已完成** | `component_attribution` 已对 dsa/technical/kronos/catalyst 做 IC；✅ 标准化 AI schema（asof_date/confidence/direction/reason_codes/warning_codes）+ 校验 + 三层 normalizer + `ai_signals.json` 已落（2026-06-17，normalizer 路线无需 flag）；缺 confidence calibration + ablation replay，见 **H3** step 2/3 |
...
## H3 — AI signal 结构化 + 归因 + ablation（ChatGPT Phase 3）— 🟡 step 1 已完成（2026-06-17）
...
2. ✅ **`replay/ai_signal_study.py`（已完成，2026-06-17）**——把标准化信封 join 候选 forward returns，按层给：
   ...
   `ai_signal_study.{json,md}`。只读、可注入 loader 离线测；405 测试通过。⏳ 剩 `replay/ai_ablation.py`
   （no-dsa/no-kronos/no-catalyst/factor-only/ai+factor 对比，第一版用已落盘 signal 做 attribution、不重跑历史 AI）。
3. 🟡 CLI：`analytics ai-signal-study` 已建；`analytics ai-ablation` 待 ablation 模块。
...
accuracy、confidence IC、reason/warning code lift；✅ 只读、不改交易行为；✅ 数据不足空层不 crash。剩 **step 3
ai-ablation**（层 ablation 对比）天然隔离、读已落盘数据、无需 flag。405 个测试通过。
