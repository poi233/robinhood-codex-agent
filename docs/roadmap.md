...
| M1–M5 intraday advisory overlay | `ENABLE_INTRADAY_ADVISORY_OVERLAY` | intraday 排序/仓位/风控 overlay | 🟡 **M1 + M4 audit skeleton 已完成（2026-06-18，默认 0）**：loader/normalizer + rankings/order audit 字段已建；M2/M3/M4 dashboard/M5 仍待接线；flag 关时 champion 完全保持旧行为 |
...
| | M2 | H2/H3 进入排序，不做硬拦 | 用户新增 | ⏳ 规划中：`factor_alpha` + AI envelopes 只调整 `trade_readiness_score`，不直接 block buy |
| | M3 | K1/K2 进入风控/仓位，只收紧 | 用户新增 | ⏳ 规划中：regime 可 block new buy 或降仓位；portfolio concentration 可 block 增仓或降仓位；绝不放大仓位 |
| | M4 | 决策日志和 dashboard 显示每个因子如何影响最终决策 | 用户新增 | 🟡 **audit skeleton 已完成（2026-06-18）**：`intraday_rankings.jsonl` 和 proposed order 已落 `advisory_overlay`；dashboard Decision Overlay 视图仍待做 |
| | M5 | growth 分析 overlay 效果并提出 paper-only 改进建议 | 用户新增 | ⏳ 规划中：让 growth 使用 calibration/factor/AI/regime/portfolio evidence 评估 overlay 权重/阈值；只提 proposal，不改 champion |
...
3. **P2 · 冻结跑 paper 15–30 天** — 不调 champion 权重/阈值（否则样本不可比），自成长只 `observe`+`propose`；
   M1 loader + M4 audit skeleton 已完成；下一步是 M2 排序接线。`ENABLE_INTRADAY_ADVISORY_OVERLAY=0` 时必须完全不改变 intraday 行为。
...
**下一步**：M4 audit skeleton（把 overlay 字段写入 rankings/decision/order intent 的空态/影子字段），再做 M2/M3 行为接线。

## M2 — H2/H3 只进入 intraday 排序
...
**Dashboard 现状判断（2026-06-18）**：
- 现有 dashboard 已有 9 个 tab：Today / Candidates / Decisions / Paper / Strategy Comparison / Calibration /
  Self-Growth / Themes / Trends。
- 已能分散展示 H2 `factor_alpha`、H3 AI study/ablation、K2 regime banner、K1 portfolio target。
- 缺口是**没有合成视图**：看不到某个 symbol 的 base score → overlay rank_delta → final score、size multiplier、
  block reason，以及 factor/AI/regime/portfolio 各自贡献。因此 M4 需要 dashboard 更新。
...
现有 9 个 tab 继续正常渲染，新 overlay 视图仍只读、不写 YAML、不触发交易。

**已完成（2026-06-18，audit skeleton）**：
...
- `_append_intraday_rankings()` 写入同一个 per-symbol `advisory_overlay`，便于 replay/dashboard 后续读取。
- 当前字段只做审计：`rank_delta=0`、`size_multiplier=1`、`block_buy=false` 时不改变任何排序、sizing 或 hard block。
- 测试：新增 intraday 集成测试确认 `trade_readiness_score` 保持不变，同时 rankings/proposed order 都带 overlay。

**仍待做**：email 中文解释、dashboard `Decision Overlay` 视图、blocked/no-action 顶层 decision overlay 汇总。
