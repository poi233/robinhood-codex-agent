...
| | H3 | AI signal 结构化 + 归因 + ablation（ChatGPT Phase 3） | ChatGPT | 🟡 **step 1 已完成（2026-06-17）**：标准化 AI 信封 schema + 校验 + 三层 normalizer + premarket 落 `ai_signals.json`（normalizer 路线，write-only advisory，**无需 flag**）；step 2/3（study/ablation replay）待做 |
...
过校验；✅ 缺产物降级空层不 crash；✅ 不改交易行为（write-only advisory）。400 个测试通过。step 2/3（study/
ablation replay + CLI）天然隔离、读已落盘数据，留作后续（像 fill_quality 一样无需 flag）。
