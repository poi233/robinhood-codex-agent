...
| **E 数据驱动校准** | E1 | replay 校准：forward/benchmark returns + 命中率 + attribution + Calibration tab | 旧 R1 + docx P1 | ⭐ **下一个代码任务（P1，现在就建机器，不等数据）** |
...
2. **P1 · 现在就建 E1 校准地基（不要等 3 周）** — **我的关键修正**：E1 的 forward returns 是用
   yfinance 对历史候选日**重算**的，候选分数 / 决策 / blocked reason 也都已落盘，所以 E1 的**机器
   并不被数据阻塞，只有统计显著性需要时间**。现在就建的好处：① 每积累一天立刻可消费；② 养成每天看
   attribution 的习惯；③ 趁早暴露数据管道 bug（就像我们刚补的 `intraday_rankings` 落盘缺口）。
   见下方扩写的 **E1**。这是**下一个最该交给 Codex 的代码任务**。
...
## E1 — replay 校准：forward/benchmark returns + 命中率 + attribution（旧 R1 + docx P1）— ⭐ 下一个代码任务
