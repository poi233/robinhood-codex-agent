...
> - **默认安全**：默认仍是 paper，`KILL_SWITCH` 阻断 review/live；review/live 只有通过 intraday Codex MCP
>   prompt 和本地门控后才能触发 Robinhood 动作，dashboard 第一版只读、不可改交易参数。
...
| **F 后期 / 部分完成** | F1 | strategy compare | docx | 依赖 B1+B2+多 strategy version |
| | F2 | review/live 真实下单接线 | 旧 R7 | 🟡 **基础接线已完成（2026-06-22）**：review/live 进入 intraday Codex MCP prompt；live 默认直接 place，dollar_amount market buy 支持；剩监控/配置/审计增强 |
...
| **P Codex 成本路由 + Technical 层混合化**（用户新增 2026-06-23） | P1 | **Codex 双层模型路由**（简单操作槽 / 需思考 full） | 用户新增（2026-06-23） | ✅ **已完成（2026-06-23）**：`prompts/codex.resolve_codex_model` 按 run_kind 选模型——简单/数据抓取/格式化（account_snapshot/market_calendar/quote_snapshot_core/catalyst_enrichment/technical_research narrative/email_*）走 `CODEX_MODEL_MINI` 槽位（当前默认 gpt-5.4，避免 mini 容量失败），需推理（final_premarket/dsa/screener/intraday/postmarket）走 `CODEX_MODEL`（默认 gpt-5.4）；`CODEX_MODEL_FORCE` 一键钉单模型；shell `common.sh` 同步同一套路由；doctor 回显两档；进度日志记 model |
| | P2 | **Technical 层混合化：确定性引擎**（决策字段全 Python，所有特征入决策） | 用户新增（2026-06-23） | ✅ **已完成（2026-06-23）**：`signals/technical_engine.build_technical_signals` 从 `technical_features` 确定性产出 action/priority/levels/setups/no_trade_zone/confidence；**多周期(周/时/15m)加权 + EMA9/21 + SMA + MACD + RSI + 多区间相对强弱(5/20/60d) + ATR(含 atr_pct 波动率) + 均量(流动性) + 全部 price-action flags 都进决策**（不再塌缩成单一 alignment 标签）；fail-closed/追高保护保留；schema 与 fallback 一致，下游零改动。**顺手修 bug**：`short_setup.status` 由 `valid`→`watch`，否则 `sell.evaluate_sell` 的 `risk_exit` 永不触发（关联 J1） |
| | P4 | **advisory overlay 接线修复**（ai_signals/portfolio 真正进决策） | 用户新增（2026-06-23） | ✅ **已完成（2026-06-23）**：修两个潜伏缺陷——① ai_signals 的 `layers` 是 envelope **列表**,但 `_ai_component`/`_symbols_from_artifacts` 当 dict 读 → AI 层 rank_delta 恒 0;② `portfolio_target` 没产 `theme_by_symbol` → 每标的 overexposed_theme 拦截永不触发。两者都在 `ENABLE_INTRADAY_ADVISORY_OVERLAY`(默认0)后,修后契约由回归测试钉住 |
| | P5 | **paper/live 决策统一**（同一确定性引擎,只差真假成交） | 用户新增（2026-06-23） | 🟢 **代码完成（2026-06-23,`ENABLE_DETERMINISTIC_INTRADAY` 默认 0）**：开启后 review/live 与 paper 共用 `generate_order_intent` 的确定性决策,只有执行不同——paper 模拟成交;review/live 由「取数 prompt(只读 MCP 拉当日账户/持仓/挂单)→ Python 决策 → 极简 execute prompt 原样下单」三段完成,LLM 仅当执行手不做分析;KILL_SWITCH/窗口/所有闸门保留,snapshot/execute 走 mini 模型。flag 关时＝现状(legacy intraday prompt 自主决策) |
| | P6 | **移除所有 feature flag,默认全开 + fundamental/event 接进决策** | 用户新增（2026-06-23） | ✅ **已完成（2026-06-23）**：除 `TRADING_MODE`(paper/review/live 开关)外,所有 `ENABLE_*` flag 从代码中删除、对应路径无条件生效——advisory overlay/deterministic intraday/各信号层/precompute/cache/batch/VIX/dynamic-active/weekly-screener/nightly/email 全部默认开。**fundamental(H7)/event(H8) 也接进 advisory overlay**(quality_flags/earnings_imminent/估值修正 → 有界 rank_delta)。至此 factor/ai/regime/portfolio/fundamental/event 六个 advisory 层全部进决策。runtime.env/common.sh/cli doctor/runtime_block 的 flag 管线一并清除;626 测试通过。注:这违背了原"大模块 flag 门控、默认关"纪律,系用户显式要求 |
| | P7 | **删除死代码 + email/dashboard 全中文化与信息增强** | 用户新增（2026-06-23） | ✅ **已完成（2026-06-23）**：删除已不被调用的 legacy `intraday/check.txt`(LLM 自主决策 prompt),safety 检查改指向新的 execute/snapshot prompt 与 policy loaders。**邮件全中文化 + 信息增强**:盘前候选现展示「技术面计划价位(入场/突破/止损/目标,来自确定性引擎)+ 量价因子/AI方向/基本面/事件 辅助层 + 催化/风险」,plan_state/market_regime/allowed_actions/setup/action_taken/各层名全部中文;盘中叠加段含 fundamental/event。**dashboard**:选股页新增「基本面与事件(H7/H8)」面板,把这两层(现已进决策)可视化;全中文。626 测试通过 |
...
| | P9 | **盘后邮件纳入影子实验盘** | 用户新增（2026-06-24） | ✅ **已完成（2026-06-24）**：postmarket 扫描当日 `experiments/*/paper/` 隔离账本，把每个 shadow strategy 的唯一成交订单数、未成交订单数和当前持仓/成本写入 paper summary，并在盘后 Gmail 增加独立「影子实验盘」段落；明确不影响主模拟盘且不会触发真实订单。 |
...
**实现进度**（分步提交）：
- ✅ **step 1 · 骨架（2026-06-19）**：`screener/` 包（`config.py` 读 5 个 env 参数 + `paths.py` 定位
...
## O3 — dashboard 选股层面板（🟢 代码完成 2026-06-19）

把 O1/O2 的只读产物搬上 dashboard 的「🎯 选股与决策」主区，回答"它选了谁/在不在跑"（描述性，**还不是**"选得对不对"——那是 O4）。

- `dashboard/queries.py`：`screener_change`（读最近一期 `universe_change.json`+`status.json`，带 `available_dates` 供选期）、
  `active_selection`（读当日 `planner/active_selection.json`）、`list_screener_dates`。
- `dashboard/charts.py`：`screener_change_view`（加/降/跳过 + 因子分 + effective 数 + 已应用/仅报告）、
  `active_selection_view`（active 构成：pins vs screen 补、ACTIVE_MAX、universe 大小）。
- `dashboard/app.py`：「选股」页加「每周选股(O1)」「今日 active 选择(O2)」两个 subheader。纯只读。
- 测试：3 个 queries 单测（不依赖 streamlit）；smoke seed 补 O1/O2 产物；**顺手修复 dashboard smoke** —— app 早已从
...
## F2 — review / live 真实下单接线（旧 R7，🟡 基础接线已完成）

**目标**：把 review/live 从本地 intent 流程接到真实 Robinhood MCP，同时保留 `KILL_SWITCH`、risk tier、
Agentic-only、allowlist、quote/tradability/account/data gate。

**已完成（2026-06-22）**：
- `orchestration/intraday.py`：`paper` 继续走确定性 Python paper broker；`review`/`live` 在 calendar/time/
  `KILL_SWITCH` gate 之后调用 `src/prompts/intraday/check.txt` 的 Codex MCP prompt。
- `live` prompt 默认不再调用 `review_equity_order`，本地门控全过后直接 `place_equity_order`；如 broker/tool
  要求 review，则 fail closed，不在无人值守 automation 里临场要求人工确认。
- 小额高价股 / 需要 fractional 的买入优先使用 regular-hours `market` + `dollar_amount` + `gfd`，避免
  Robinhood 拒绝 `fractional quantity + limit`。
- `review` 模式仍只审单不下单；`--dry-run` 在非 paper 模式跳过 Codex MCP prompt，避免测试误触真实下单。

**前置条件（硬性）**：
- 人工显式移除 `KILL_SWITCH`，人工设 `TRADING_MODE=live` 和 `RISK_TIER`（绝不让 Codex 自动改）。
- launchd/Codex OAuth/Robinhood MCP 必须健康；`python -m trading_agent doctor` 必须显示预期 runtime。
...
**剩余步骤**：
1. 增加 live order audit 结构化日志校验：submitted/queued/filled/rejected/canceled、broker error、remaining buying power。
2. 增加成交质量监控：dollar market buy 的实际成交价、滑点、spread、queued→filled latency。
3. 增加显式运行开关：例如 `ENABLE_LIVE_DOLLAR_MARKET_BUY=1`、`REQUIRE_ROBINHOOD_REVIEW=1` 的 doctor 回显。
4. 用 tier 0/1 小单连续观察日志，确认不会重复下单、不会越过 daily cap、不会在非 regular hours 误下 dollar-based order。
...
## H4 — factor/analyzer/setup shadow 策略（ChatGPT Phase 4 增量）— 🟡 多权重 re-score + setup-shadow 已建（2026-06-23）

**已完成（setup-shadow，2026-06-23）**：补上了一直缺的「setup 开关」那一半。买入价格门 `decide_buy_price`
改成 `policy/setups.py:SETUP_REGISTRY` 的分发器——策略经 `policy_profile["setups"]` 选择入场逻辑，champion 不配置
→ 默认 `pullback`→`breakout`（逐字节不变）。新增 4 个从**始终存在的 key_levels** 派生入场的 setup
（`breakout_momentum` / `trend_continuation` / `dip_pullback` / `range_reversion`），不依赖 premarket 那个
非看涨时塌成零宽的 `long_setup.entry_zone`，所以 champion 沉默的日子也能成交。`shadow_runner` 现按 experiment 的
`policy_profile` 字段给每个 challenger 换上自己的 setups+阈值；`sizing_policy._score_multiplier` 的分档阈值改成
profile 可配（默认=champion 93/88/82，新 profile 设更低档位才会变）。4 套已作为 `active_shadow` 种入
`strategy_experiments.yaml`，各自隔离账本，经 `growth recommend` + dashboard 对比。champion 冻结、只 paper/影子。
（见 `policy_profiles.json` 的 momentum_breakout/trend_follow/dip_pullback/range_reversion + `growth_policy.json`
setups 白名单扩展。）
...
- ✅ **doc/code 一致**：`strategy.md` 删掉「down >3% → 只 alert」的错误描述，改成准确的自动退出说明
  （technical invalidation 全平 + 8% 兜底硬止损由 paper broker 执行；review/live 通过 intraday Codex MCP prompt
  走真实 broker gate）。
...
- review/live 已接真实 MCP prompt；paper/shadow 仍需先跑 15–30 个交易日再考虑把自成长建议人工 promote 到
  champion 参数。
...
## Q — 策略发现与多样性引擎（最大化复用历史数据筛策略）— ✅ 全部完成（2026-06-24，Q1–Q6）

> **核心反转**：现状是「先想一个 setup → 前向 paper 慢慢攒数据」。Q 段把它倒过来——**先拿全部历史数据
> 批量筛，再把幸存者放进前向 paper**，只有前向确认后才人工提拔 champion（G8 流程不变）。多样性来自
> **基因轴的组合**（选股 × 入场 × 出场 × 风险 × regime × 时段），不是堆同质 setup；并用可度量的
> **两两相关性**保证「覆盖数据的不同部分」而非攒一堆 breakout 变体。
>
> **隔离纪律**：Q1/Q2/Q4/Q5 是**离线只读分析**（同 replay/analytics，天然隔离、不进热路径 → 不需 flag）；
> Q3 新 setup 只进 challenger 的 `policy_profile`（shadow-only 隔离账本、不碰 champion，同 H4）；只有 Q6 采集
> 日内 bar 会接 intraday 热路径 → **必须 `ENABLE_INTRADAY_BAR_CAPTURE` flag、默认 0**（遵守贯穿全局的 flag 约定）。

### Q1 — 历史 setup 重放筛选器（harness）— ✅ 已完成（2026-06-24）
**已完成（2026-06-24）**：`replay/setup_screen.py` + CLI `analytics setup-screen`。对每个历史 run_date 用
`load_policy_inputs` 还原 point-in-time `PolicyInputs`（候选/key_levels/quote/profile），跑真实 `rank_candidates`
→ `decide_buy_price`，把每个非 blocked 决策当假设入场；只在判 target/stop 时才看未来 daily bar。两种模式：默认
`screen_all_setups`（每个注册 setup **独立**跑，head-to-head 出「setup × 胜率 × 平均前向收益 × 样本」表），或
`--setups/--profile` 筛某个 setup 栈（first-clears-wins）。中性化 champion 组合状态（持仓/挂单/usage）使各 setup
edge 可比，但保留真实 point-in-time 闸门。只读、写 `runtime/analytics/setup_screen.{json,md}`；无数据/无成交都优雅
降级；接进夜间批。5 个单测（`_evaluate_fills` 精确判定 + 端到端 mock + no_data）。
**目标**：输入任意 setup 配置，把它重放过**全部历史 run_date** 的候选 universe + key_levels，算出假设成交 +
前向收益（target-first/stop-first/Nd 收益）+ 样本数，一条命令吐出「setup × 胜率 × 前向收益 × N」表。这是 Q 段
其余项的地基——把「猜 setup → 等几周 paper」变成「几秒筛 N 个 → 只前向跑幸存者」。
**具体**：复用 `replay/forward_returns.py`（`PriceLoader`/`compute_forward_return_records`/`_entry_index`）
+ `replay/analysis.py`（`discover_run_dates`）；新增 `replay/setup_screen.py` 把历史候选 + key_levels 喂给
`policy/setups.py:SETUP_REGISTRY` 的纯函数（point-in-time：只用当日及之前数据），产假设 BuyPriceDecision →
按 forward bars 判 target/stop。CLI `analytics setup-screen [--setups ...] [--since/--until]`。
**涉及文件**：新增 `replay/setup_screen.py` + `tests/.../test_setup_screen.py`；`cli.py`（命令 + 回显）。
...
**具体**：新增 `growth/diversity.py`（相关性/重叠/贪心选择纯函数，读 shadow_decisions + 隔离账本 equity）；
接进 `growth/evaluator.py` 的 recommend 路径（加 min-trade gate + 相关性列）。
...
**具体**：在 `policy/setups.py` 注册 3 个新 setup 纯函数（从 key_levels + benchmark bars 派生，沿用同样的
价格图/min_reward_risk 硬安全）；每个先用 Q1 harness 历史筛，**过了**才加 `policy_profiles.json` profile +
`strategy_experiments.yaml` 种 active_shadow + `growth_policy.json` setups 白名单。
**涉及文件**：改 `policy/setups.py`（+测试）；过筛后改 `policy_profiles.json`/`strategy_experiments.yaml`/`growth_policy.json`。
...
gap%/RVOL/距支撑/regime 分桶，量每桶前向收益 → 有 edge 但无 setup 吃到的桶 = 新策略规格）。
**具体**：新增 `replay/discovery.py`（三个离线分析，复用 near_miss + forward_returns + 候选特征）；CLI
`analytics discover`，产 markdown 报告（候选 setup 假设清单），人工/Q3 接力实现。
...
**具体**：扩 Q1 harness 加 `--train-until/--test-since`；新增 `replay/significance.py`（FDR/deflated Sharpe/
组合净值纯函数），接进 setup-screen 与 recommend。
**涉及文件**：改 `replay/setup_screen.py`；新增 `replay/significance.py` + 测试。
**验收**：同一组 setup 的 train/test 表分开出；p 值经 FDR 调整后给「显著/不足」标注；组合净值含相关性体现。

### Q6 — 日内 bar 采集 + 日内 setup（flag 门控）— ✅ 已完成（2026-06-24）
**已完成（2026-06-24）**：① **日内 bar 采集**`data/intraday_bars.py`(`capture_intraday_bars`/`load_intraday_bars`)
+ intraday 热路径里 `_maybe_capture_intraday_bars` 在 `ENABLE_INTRADAY_BAR_CAPTURE=1`(默认 0)时把每个 tick 的
per-symbol 价格追加到 `runtime/state/runs/<date>/intraday_bars.jsonl`;flag 关＝完全 no-op、零行为变化;best-effort
不阻塞交易;doctor 回显该 flag。② `PolicyInputs.intraday_bars` 新字段 + `load_policy_inputs` 加载(additive,无文件
即空)。③ 两个日内 setup:`opening_range_breakout`(开盘前 N 根的区间高点突破)、`vwap_reclaim`(价格回踩重夺 session
均价)——读 `inputs.intraday_bars`,无 bar 时优雅 `insufficient_intraday_bars`。各加 `policy_profiles.json` profile +
`growth_policy.json` 白名单(共 11 个 setup)。9 个新单测(采集 on/off + 加载 + 两 setup fires/blocks)。
**设计说明**：(a) **未自动 seed shadow 实验**——这两个 setup 在 flag 开启采集前一直 `insufficient_intraday_bars`,
为免 `growth recommend` 多两个恒 block 的挑战者,profile/registry/白名单已就绪,但等你开了 `ENABLE_INTRADAY_BAR_CAPTURE`
跑出日内数据后再手动种 active_shadow。(b) **vwap_reclaim 是纯价格近似**——真 VWAP 需要成交量,当前只采价格快照,
用 session 均价(≈TWAP)代替,docstring 已注明;后续给采集加 volume 即可升级为真 VWAP。(c) Q1 的 harness 目前用日线
PriceLoader,**日内回测**需要一个读 intraday_bars 的 intraday loader,留作下一步(数据已开始采集就能补)。
**目标**：抬掉「只有日收盘价」的天花板（`setup_outcomes` 注明 daily-close 低估日内触及）。采集日内 bar/快照后，
解锁 `opening_range_breakout`（开盘区间突破）、`vwap_reclaim`（VWAP 回踩重夺）两个**日内择时** setup，并让 Q1
harness 能在日内 bar 上回测它们。
**具体**：`ENABLE_INTRADAY_BAR_CAPTURE`（默认 0，**接 intraday 热路径故强制 flag**）下，intraday 落 1–5min
bar/快照到 `runtime/.../intraday_bars/`；harness 增日内 PriceLoader；新增两个 setup。
**涉及文件**：intraday 采集接入点（flag 包住）+ 新 loader + `policy/setups.py` 两个 setup + 测试；doctor 回显 flag。
**验收**：flag=0 时 intraday 逐字现状、零落盘；flag=1 时只**新增**日内 bar 产物、不改决策；两个 setup 仅在有日内
bar 时可筛；doctor 显示开关。
