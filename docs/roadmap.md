...
| | H1 | 校准补强（21/63d horizon · per-candidate 超额收益 · 多 horizon Rank IC + t-stat · 更全 near-miss） | ChatGPT P1 残项 | ⏳ 可现在做（小，扩展现有模块） |
| | H2 | **价量因子层**（factors_price + factor_alpha + factor_profiles，ChatGPT Phase 2） | ChatGPT | ⭐ **最高价值净新增 · 下一个代码任务**（关键：立刻接进 premarket 落盘攒数据） |
| | H3 | AI signal 结构化 + 归因 + ablation（ChatGPT Phase 3） | ChatGPT | 🟡 归因半成（component IC 已覆盖 dsa/tech/kronos/catalyst）；schema 标准化需先做以攒数据 |
| | H4 | factor/analyzer/setup shadow 策略（ChatGPT Phase 4 增量） | ChatGPT | ⏳ 依赖 H2 + premarket re-score 路径 |
| | H5 | dashboard calibration 子视图扩展（ChatGPT Phase 5 增量） | ChatGPT | 🟢 主 tab 已建（C3 第 8 Tab）；子视图随 H2/H3 数据增量加 |
...
| **I 运维与自动化**（**仅写 roadmap，暂不执行**） | I1 | 夜间分析/自成长自动化 cron（收盘后自动跑 analytics/calibrate/growth） | 用户新增 | ⏳ 可现在做（小、加法式、只读/shadow-only） |
| | I2 | 每天一份分析快照（`history/<date>/` + nightly_summary.json） | 用户新增 | ⏳ 可现在做 |
| | I3 | 拿到趋势的功能（`analytics trend` + `build_trend` 纯函数） | 用户新增 | ⏳ 可现在做 |
| | I4 | dashboard 可视化（新鲜度 + 日期回看每天结果 + 趋势折线） | 用户新增 | ⏳ 依赖 I2/I3 |
| **J 评审驱动修正（2026-06-16 外部评审）** | J1 | 止损/退出逻辑校正 + strategy.md 一致性（**live 硬前置**） | 评审 | ⏳ 重要（live 前必修；诚实 paper 校准也需要） |
| | J2 | Codex-facing 文档旧路径统一（risk.md / strategy.md → dated run folder） | 评审 | ⏳ 小、可现在做（防误导 Codex/自己） |
...
7. **新方向 · H 阶段（量化因子 + AI 归因）** ← **下一个代码任务在这**：评估了用户带来的 ChatGPT 计划，
   Phase 1（校准地基）/ shadow 隔离账本 / dashboard calibration tab **本会话已做掉一大半**，真正缺的是
   **价量因子层（H2）**。**下一个代码任务 = H2（第一版只落盘 + paper 尽早开 `ENABLE_PRICE_FACTOR_LAYER=1` 攒
   数据）**，并行 H1 校准补强；详见文末 **H 阶段**。
...
8. **外部评审确认（2026-06-16）+ 收敛后的推荐序**：一次外部评审确认了方向（paper-only / 安全边界 / H2 为
   ...
   **稳定 paper 10–30 天（冻结 baseline）→ H2 因子层只落盘 → H1 校准补强 → E4 成交质量 → 再 shadow
   challenger / 权重调整**。另：J2 文档旧路径统一是个可现在做的小修。
...
| Phase 1 的 21/63d horizon、per-candidate 超额收益、多 horizon Rank IC + t-stat、更全 near-miss 类别 | ⏳ **缺口** | 见 **H1**（现有模块只有 1/3/5d、单 horizon Spearman IC、near-threshold near-miss） |
| **Phase 2 价量因子层** | ❌ **没有** | **真正缺的一条腿**，见 **H2**（最高价值净新增） |
| **Phase 3 AI signal study** | 🟡 **半成** | `component_attribution` 已对 dsa/technical/kronos/catalyst 做 IC（= 「AI score vs forward return」）；缺标准化 AI schema（confidence/reason_codes/warning_codes）+ confidence calibration + ablation，见 **H3** |
...
| **Phase 5 dashboard calibration tab** | 🟢 **主 tab 已建** | C3 第 8 个 Tab「Calibration」已在；ChatGPT 列的 factor coverage / factor IC / AI calibration 子视图依赖 H2/H3 数据，见 **H5** |
...
## H1 — 校准补强（ChatGPT Phase 1 残项）— ⏳ 可现在做（小）
...
## H2 — 价量因子层（ChatGPT Phase 2）— ⭐ 最高价值净新增 · 下一个代码任务
...
## H3 — AI signal 结构化 + 归因 + ablation（ChatGPT Phase 3）— 🟡 半成

**现状**：`component_attribution` 已对 dsa/technical/kronos/catalyst/quote 算 forward-return IC（= ChatGPT
要的「AI score vs forward return」「各 AI 层 attribution」的核心）。**缺的是**：
1. **标准化 AI 输出 schema**（每层带 `asof_date`/`confidence`/`direction`/`reason_codes`/`warning_codes`/
   `time_horizon`/`risk_flags`）——这是 prompt-contract 改动，**应尽早做以开始积累**（confidence/reason_codes
   今天不存今天就没有）。
2. `replay/ai_signal_study.py`（confidence calibration、reason_code/warning_code 效果）+ `replay/ai_ablation.py`
   （no-dsa/no-kronos/no-catalyst/factor-only/ai+factor 对比，第一版用已落盘 signal 做 attribution、不重跑历史 AI）。
3. CLI `replay ai-signal-study` / `replay ai-ablation`（或并入 `analytics`）。
...
## H5 — dashboard calibration 子视图扩展（ChatGPT Phase 5 增量）— 🟢 主 tab 已建

主 Calibration Tab（C3 第 8 个）已在。随 H2/H3 数据落地，**增量**加子视图：factor coverage、factor
values、factor IC、AI calibration（score/confidence/reason/warning）、（setup outcomes / near-miss / 桶收益
/ champion-vs-challenger 已在）。只读、headless render test、不改 YAML、不触发交易。
...
1. **H2 价量因子层（第一版只落盘 + 进 calibration）** ← 下一个代码任务，最高价值，且立刻接 premarket 攒数据。
2. **H1 校准补强**（21/63d horizon + 逐候选超额 + 多 horizon Rank IC/t-stat）——小、和 H2 并行做，让因子一
   落盘就能被多 horizon IC 校准。
...
> **I4** dashboard 可视化（每天结果 + 趋势）。**当前全部仅写入 roadmap，暂不实现。**
>
...
## I1 — 夜间分析/自成长自动化 cron — ⏳ 仅写 roadmap
...
## I3 — 拿到趋势的功能（"一个功能可以拿到这个趋势"）— ⏳ 仅写 roadmap
...
## I4 — dashboard 可视化（每天结果 + 趋势）— ⏳ 仅写 roadmap
...
## J1 — 止损/退出逻辑校正 + strategy.md 一致性 — ⏳ 重要（**live 硬前置**）
...
## J2 — Codex-facing 文档旧路径统一 — ⏳ 小、可现在做

**问题（已核实）**：`src/config/risk.md` / `strategy.md` 仍写**旧的扁平路径**——`runtime/state/today_allowlist.txt`、
`runtime/state/daily_plan.json`、`runtime/state/dsa_signals.json` 等；而系统实际用**按日期的 run folder**
（`runtime/state/runs/<date>/planner/today_allowlist.txt`、`.../daily_plan.json`、`.../signals/dsa_signals.json`）。
这两个文件是 **premarket Codex prompt 会读的 config**，旧路径会误导 Codex（和人）。

**具体步骤**：把 `risk.md` / `strategy.md` 里的 `runtime/state/<file>` 全部更新为现行 dated-run-folder 路径
（或改成用 runtime block 里的 `*_PATH` 变量名描述，避免再写死路径）；与 README / `core/context.py` 的
`RuntimePaths` 对齐。

**涉及文件**：`src/config/risk.md`、`src/config/strategy.md`（纯文档；不动代码）。

**验收**：两个 config doc 不再出现旧 `runtime/state/<flat>` 路径；与实际 `RuntimePaths` / README 一致。
