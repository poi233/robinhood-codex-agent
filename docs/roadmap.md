...
| **P Codex 成本路由 + Technical 层混合化**（用户新增 2026-06-23） | P1 | **Codex 双层模型路由**（简单操作 mini / 需思考 full） | 用户新增（2026-06-23） | ✅ **已完成（2026-06-23）**：`prompts/codex.resolve_codex_model` 按 run_kind 选模型——简单/数据抓取/格式化（account_snapshot/market_calendar/quote_snapshot_core/catalyst_enrichment/technical_research narrative/email_*）走 `CODEX_MODEL_MINI`（默认 gpt-5.4-mini），需推理（final_premarket/dsa/screener/intraday/postmarket）走 `CODEX_MODEL`（默认 gpt-5.4）；`CODEX_MODEL_FORCE` 一键钉单模型；shell `common.sh` 同步同一套路由；doctor 回显两档；进度日志记 model |
