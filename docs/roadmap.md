...
| M1–M5 intraday advisory overlay | `ENABLE_INTRADAY_ADVISORY_OVERLAY` | intraday 排序/仓位/风控 overlay | 🟡 **M1–M5 evidence core 已完成（2026-06-18，默认 0）**：loader/normalizer + H2/H3 rank_delta 排序 + K1/K2 风控/仓位收紧 + rankings/order/email/dashboard audit + forward-return/growth evidence 已建；M5 proposal 调参仍待；flag 关时 champion 完全保持旧行为 |
...
| | M5 | growth 分析 overlay 效果并提出 paper-only 改进建议 | 用户新增 | 🟡 **evidence core 已完成（2026-06-18）**：forward returns/calibration 自动纳入 overlay components，growth evidence 能读取 overlay IC；paper-only proposal mutation 仍待 |
...
3. **P2 · 冻结跑 paper 15–30 天** — 不调 champion 权重/阈值（否则样本不可比），自成长只 `observe`+`propose`；
   M1–M5 evidence core 已完成；下一步是 M5 proposal mutation 白名单/validator。`ENABLE_INTRADAY_ADVISORY_OVERLAY=0` 时必须完全不改变 intraday 行为。
...
- 测试：forward returns overlay pickup、growth evidence overlay IC。

**仍待做**：`growth_policy.json` overlay mutation 白名单、validator 支持、proposal rule 只在 evidence 支撑时提出
paper-only overlay 调参。
