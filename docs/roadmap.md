...
| M1–M5 intraday advisory overlay | `ENABLE_INTRADAY_ADVISORY_OVERLAY` | intraday 排序/仓位/风控 overlay | 🟡 **M1/M2/M3 + M4 audit skeleton 已完成（2026-06-18，默认 0）**：loader/normalizer + H2/H3 rank_delta 排序 + K1/K2 风控/仓位收紧 + rankings/order audit 字段已建；M4 dashboard/M5 仍待接线；flag 关时 champion 完全保持旧行为 |
...
3. **P2 · 冻结跑 paper 15–30 天** — 不调 champion 权重/阈值（否则样本不可比），自成长只 `observe`+`propose`；
   M1/M2/M3 + M4 audit skeleton 已完成；下一步是 M4 dashboard/email 可解释性。`ENABLE_INTRADAY_ADVISORY_OVERLAY=0` 时必须完全不改变 intraday 行为。
