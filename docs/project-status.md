...
| 组合管理 / 市场 regime 引擎 / thesis 归因 | 🟡 **K1+K2+K3 第一版全部已建（2026-06-17）**：K1 `portfolio/target.py`（cash/单仓/主题敞口 vs 上限）；K2 `regime/engine.py`（确定性 regime + 仓位乘子，applied=min(1.0,·) 只降风险）；K3 `replay/thesis.py` + `analytics thesis`（thesis 标签 join forward returns → 主题级胜率）。K1/K2 premarket advisory write-only、K3 只读 replay；均 dashboard 显示、绝不加买入/不接 sizing。第二版（接 sizing/落 thesis 标签）待校准 |
