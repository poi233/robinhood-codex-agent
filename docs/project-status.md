...
| 组合管理 / 市场 regime 引擎 / thesis 归因 | 🟡 **K1+K2 第一版已建（2026-06-17）**：K1 `portfolio/target.py`（cash/单仓/主题敞口 vs 上限 + 超限 flag）；K2 `regime/engine.py`（确定性 bull/neutral/risk_off/panic + 仓位乘子，applied=min(1.0,·) 只降风险）；均 premarket advisory write-only、dashboard 显示、绝不加买入/不接 sizing。**剩 K3 thesis 归因**，见 roadmap **K 阶段** |
