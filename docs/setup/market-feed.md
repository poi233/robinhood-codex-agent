...
The technical layer depends on `runtime/state/runs/<date>/market_feed/manifest.json`. If market feed data is
...
```text
runtime/state/runs/<date>/market_feed/
  manifest.json
  charts/
  ohlcv/
  news/
...
MARKET_FEED_DIR=runtime/state/runs/2026-06-14/market_feed
```

`src/scripts/lib/common.sh` loads `src/config/runtime.env` and then `src/config/runtime.env.local`; environment
...
```bash
CODEX_EXEC_DRY_RUN=1 ./src/scripts/data/run_market_feed_collection.sh
```
...
- Dry-run premarket records stage status in `runtime/logs/runs/<date>/pipeline.jsonl`.
