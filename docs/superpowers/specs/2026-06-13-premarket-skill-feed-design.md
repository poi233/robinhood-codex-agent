...
- write a normalized artifact bundle under `state/market_feed/<date>/`
...
This is a dedicated, separate step. It must not be folded back into `prompts/premarket_research.txt`.
...
- write `state/technical_signals.json`

Inputs:

- `state/market_feed/<date>/charts/...`
- `state/market_feed/<date>/ohlcv/...`
- `state/market_feed/<date>/news/...`
- `.agents/skills/...`
- optional `state/dsa_signals.json`
...
- operator runs `scripts/run_symbol_research.sh SYMBOL`
...
1. `state/market_feed/<date>/manifest.json`
2. `state/technical_signals.json`
