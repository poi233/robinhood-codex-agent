...
Technical price levels must be carried into final outputs for the intraday trader. They must not remain only inside `state/technical_signals.json`.
...
4. Move market-feed collection code into `trading_agent.data` and keep `scripts/collect_market_feed.py` as a wrapper.
