...
### 6. `scripts/verify_kronos_env.sh`
...
2. `scripts/setup_kronos_env.sh` creates `.venv-kronos`, clones `.vendor/kronos`, installs dependencies, and writes local env overrides.
3. `scripts/verify_kronos_env.sh` validates the environment.
4. Operator manually completes Codex login, Robinhood MCP setup, and Robinhood desktop authentication.
5. `scripts/run_premarket.sh` starts.
6. `dsa_premarket_scan` runs if enabled.
7. `kronos_premarket_scan` runs if enabled.
8. `scripts/kronos_generate_signals.py` writes `state/kronos_signals.json`.
9. `prompts/premarket_research.txt` reads DSA and Kronos signal files when available.
...
4. Add `scripts/kronos_generate_signals.py` with mock output first.
5. Add `scripts/run_kronos_premarket_scan.sh`.
6. Update `scripts/run_premarket.sh` to call the Kronos layer after DSA and before main premarket research.
7. Update `prompts/premarket_research.txt` to read and constrain Kronos signals.
8. Update `README.md`, setup docs, and `scripts/check_safety.sh`.
