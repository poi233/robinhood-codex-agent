...
**Architecture:** Keep `.agents/skills/` as the repository source of truth, copy those skills into both `$HOME/.agents/skills` and `~/.codex/skills`, preserve the current `DSA -> Kronos -> main premarket` ordering, insert normalized market artifacts under `state/market_feed/<date>/` after the Kronos layer, run a separate Codex technical-analysis prompt that writes `state/technical_signals.json`, then let the main premarket and intraday prompts consume that contract while preserving long-only execution rules.
...
- Create: `prompts/technical_research.txt`
- Create: `scripts/install_repo_skills.sh`
- Create: `scripts/verify_repo_skills.sh`
- Create: `scripts/collect_market_feed.py`
- Create: `scripts/run_market_feed_collection.sh`
- Create: `scripts/run_technical_research.sh`
- Create: `scripts/run_symbol_research.sh`
...
- Modify: `scripts/check_safety.sh`
- Modify: `scripts/common.sh`
- Modify: `scripts/run_premarket.sh`
- Modify: `prompts/intraday_check.txt`
- Modify: `prompts/premarket_research.txt`

## Responsibility Split

- `.agents/skills/**`: repo-owned skill source of truth.
- `scripts/install_repo_skills.sh`: copy repo skills into both user-level skill destinations.
- `scripts/verify_repo_skills.sh`: verify installed skills are complete copies.
- `scripts/common.sh`: shared runtime flags, helper paths, and prompt runtime block fields.
- `scripts/collect_market_feed.py`: deterministic artifact collector for OHLCV, charts, news, earnings, and filings.
- `scripts/run_market_feed_collection.sh`: shell entrypoint for scheduled collector runs.
- `prompts/technical_research.txt`: explicit technical-analysis prompt that uses repo skills and writes `state/technical_signals.json`.
- `scripts/run_technical_research.sh`: scheduled and manual runner for the technical-analysis step.
- `prompts/premarket_research.txt`: consume technical signals instead of re-deriving all raw chart interpretation.
- `prompts/intraday_check.txt`: consume key price levels and no-trade zones from `state/technical_signals.json`.
- `scripts/run_symbol_research.sh`: ad hoc single-symbol collector + technical-analysis path.
...
Create `scripts/install_repo_skills.sh` with:
...
Create `scripts/verify_repo_skills.sh` with:
...
```bash
git add .agents/skills scripts/install_repo_skills.sh scripts/verify_repo_skills.sh tests/test_install_repo_skills.py
...
TECHNICAL_SIGNALS_FILE=state/technical_signals.json
```

- [ ] **Step 5: Export new shared paths from `scripts/common.sh`**
...
TECHNICAL_SIGNALS_FILE="${TECHNICAL_SIGNALS_FILE:-state/technical_signals.json}"
...
```bash
git add .gitignore config/runtime.env scripts/common.sh tests/test_install_repo_skills.py
...
Expected: FAIL on missing `scripts/collect_market_feed.py`.

- [ ] **Step 3: Implement the collector module with deterministic mock mode**

Create `scripts/collect_market_feed.py` with this structure:
...
Create `scripts/run_market_feed_collection.sh` with:
...
"$MARKET_FEED_PYTHON_BIN" "$AGENT_ROOT/scripts/collect_market_feed.py" \
...
```bash
git add scripts/collect_market_feed.py scripts/run_market_feed_collection.sh tests/test_collect_market_feed.py
...
Expected: FAIL because `prompts/technical_research.txt` is missing.

- [ ] **Step 3: Write the technical research prompt**

Create `prompts/technical_research.txt` with:
...
- `config/strategy.md`
- `config/runtime.env`
- `state/dsa_signals.json` if it exists
...
Write `state/technical_signals.json` using the exact schema in the repo design spec.
Print a short stdout summary.
```

- [ ] **Step 4: Add the technical research runner**

Create `scripts/run_technical_research.sh` with:
...
```bash
git add prompts/technical_research.txt scripts/run_technical_research.sh tests/test_technical_signal_schema.py
...
**Files:**
- Modify: `scripts/run_premarket.sh`
- Modify: `scripts/run_intraday.sh`
- Modify: `scripts/check_safety.sh`
- Modify: `prompts/premarket_research.txt`
- Modify: `prompts/intraday_check.txt`
...
- [ ] **Step 3: Update `scripts/run_premarket.sh` to insert the new steps**
...
if [[ "${ENABLE_MARKET_FEED_LAYER:-1}" == "1" ]]; then
  "$AGENT_ROOT/scripts/run_market_feed_collection.sh"
fi

if [[ "${ENABLE_TECHNICAL_SIGNAL_LAYER:-1}" == "1" ]]; then
  "$AGENT_ROOT/scripts/run_technical_research.sh"
fi
...
Add to `prompts/premarket_research.txt` under the read list:

```text
- `state/technical_signals.json` if it exists and is for today
```

Add a new task rule:

```text
Treat `state/technical_signals.json` as a non-binding but execution-aware research layer:
...
Add to the read list in `prompts/intraday_check.txt`:
...
Add to `scripts/check_safety.sh`:

```bash
if [[ -f "$AGENT_ROOT/prompts/technical_research.txt" ]] \
  && rg -q 'state/technical_signals.json' "$AGENT_ROOT/prompts/premarket_research.txt" \
  && rg -q 'state/technical_signals.json' "$AGENT_ROOT/prompts/intraday_check.txt"; then
...
```bash
git add scripts/run_premarket.sh scripts/check_safety.sh prompts/premarket_research.txt prompts/intraday_check.txt tests/test_technical_signal_schema.py
...
Expected: FAIL on missing `scripts/run_symbol_research.sh`.

- [ ] **Step 3: Create the manual symbol runner**

Create `scripts/run_symbol_research.sh` with:
...
MARKET_FEED_DIR="$manual_dir" run_codex_prompt "technical_research" "$AGENT_ROOT/prompts/technical_research.txt"
```
...
```bash
git add scripts/run_symbol_research.sh tests/test_collect_market_feed.py
...
Run: `CODEX_EXEC_DRY_RUN=1 ./scripts/run_premarket.sh`
...
- Type consistency:
  - `state/technical_signals.json` uses `long_setup`, `short_setup`, and `no_trade_zone` consistently
