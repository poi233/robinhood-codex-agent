...
COLLECTOR = REPO_ROOT / "src" / "scripts" / "data" / "collect_market_feed.py"
...
class MarketFeedCollectorTests(unittest.TestCase):
    ...
    def test_run_symbol_research_script_exists(self) -> None:
        self.assertTrue((REPO_ROOT / "src" / "scripts" / "data" / "run_symbol_research.sh").exists())

    def test_runner_uses_mock_collection_in_dry_run_mode(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            (tmp / "src" / "config").mkdir(parents=True)
            (tmp / "src" / "scripts" / "data").mkdir(parents=True)
            (tmp / "src" / "scripts" / "lib").mkdir()
            ...
            (tmp / "src" / "scripts" / "data" / "run_market_feed_collection.sh").write_text(
                (REPO_ROOT / "src" / "scripts" / "data" / "run_market_feed_collection.sh").read_text(encoding="utf-8"),
                encoding="utf-8",
            )
            (tmp / "src" / "scripts" / "data" / "collect_market_feed.py").write_text(
                COLLECTOR.read_text(encoding="utf-8"),
                encoding="utf-8",
            )
            shutil.copytree(REPO_ROOT / "src" / "trading_agent", tmp / "trading_agent")

            result = subprocess.run(
                ["bash", "src/scripts/data/run_market_feed_collection.sh"],
                cwd=tmp,
            ...
            self.assertEqual(result.returncode, 0, msg=result.stderr)
            manifests = list((tmp / "runtime" / "state" / "runs").glob("*/market_feed/manifest.json"))
